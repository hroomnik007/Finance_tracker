...
### UI Fixes
- **Category mobile list**: mobile category list now shows spent amount, progress bar and percentage — matches desktop card style. Uses `budgetStatuses.find(b => b.categoryId === cat.id)` same as desktop.
- **Motivational message**: dynamic savings percentage in 5% steps — `Math.floor(pct / 5) * 5` — message shows e.g. "Ušetrili ste viac ako 60 %" instead of hardcoded 30 %
- **Month switcher**: prev arrow disabled when at `tracking_start_date ?? createdAt` — user cannot navigate before registration date
- **Statistics**: chart filters out months before tracking start date

### Performance
- **Dashboard memoization**: `pieData`, `last5`, `last5Income` wrapped in `useMemo` — no O(n) recalculation on every render
- **FixedExpenses filter**: `filtered` list wrapped in `useMemo([fixedExpenses, activeCat])`
- **VariableExpenses filter**: inline filter/sort/reduce wrapped in `useMemo`
- **Streak parallel queries**: `updateStreakAndBadges` uses `Promise.all` for user + txCount queries — sequential → parallel
- **Resize debounce**: resize event listener in App.tsx debounced 150ms — no re-render on every pixel

### Repo rename
- GitHub repo renamed: `Finance_tracker` → `Finvu` (https://github.com/hroomnik007/Finvu)
- Gitea repo renamed: `Finance_tracker` → `Finvu`
- Server git remote `origin` updated to new GitHub URL
- Local git remotes updated on dev machine
- `feature/redesign` branch deleted from local and Gitea — fully merged into main

### Banned patterns (additions)
- No WebAuthn / biometrics for app-lock — removed, not reliable in PWA
- No localStorage for PIN hash — server is source of truth (users.pin_hash)
- No usePinLock() called directly in components — always use usePinLockContext()
- No hooks after conditional returns — React error #310
