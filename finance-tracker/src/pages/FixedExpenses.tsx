...
export function FixedExpensesPage({ month, year, onMonthChange }: FixedExpensesPageProps) {
  ...
  async function handleSave() {
    ...
    if (editing?.id != null) {
      await updateFixedExpense(editing.id, { label: label.trim(), amount: amt, dayOfMonth: day })
    } else {
      await addFixedExpense({ label: label.trim(), amount: amt, dayOfMonth: day })
    }
    closeSheet()
  }
          ...
          {fixedExpenses.length === 0 ? (
            ...
            <div>
              {fixedExpenses.map((expense, idx) => (
                ...
                >
                  <div className="hidden sm:flex w-10 h-10 rounded-2xl items-center justify-center shrink-0" style={{ backgroundColor: 'rgba(148,163,184,0.12)' }}>
                    <Lock size={16} style={{ color: '#B8A3E8' }} />
                  </div>
                  <div className="flex items-center justify-center w-9 h-9 rounded-xl shrink-0" style={{ backgroundColor: 'rgba(167,139,250,0.12)', border: '1px solid rgba(167,139,250,0.2)' }}>
                    <span className="text-xs font-bold text-[#A78BFA]">{ordinalSuffix(expense.dayOfMonth)}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[#E2D9F3] truncate leading-snug">{expense.label}</p>
                    <p className="text-xs text-[#9D84D4] mt-0.5 truncate">{t.expenses.fixed.everyMonth}, {ordinalSuffix(expense.dayOfMonth)}</p>
                  </div>
                  <span className="font-mono font-semibold text-[#f87171] shrink-0">{formatAmount(expense.amount)}</span>
                  <div className="hidden sm:flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    <button onClick={() => openEdit(expense)} className="btn-icon text-[#9D84D4] hover:text-[#A78BFA]">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => setDeleteId(expense.id!)} className="btn-icon text-[#9D84D4] hover:text-[#f87171]">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
              ...
              >
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#9D84D4]">{t.expenses.fixed.totalLabel}</p>
                  <p className="text-xs text-[#B8A3E8] mt-0.5">{t.expenses.fixed.everyMonth}</p>
                </div>
                <span className="font-mono text-xl font-bold text-[#E2D9F3]">{formatAmount(total)}</span>
              </div>
            </div>
          )}
        </div>
